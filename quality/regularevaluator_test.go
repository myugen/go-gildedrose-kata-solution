...
func TestRegularEvaluator_Evaluate(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			args: args{item: item.Item{Name: "Regular-evaluated item", SellIn: 1, Quality: 9}},
			want: want{quality: 8},
		},
		{
			name: "should decrease by 2 the quality value when an item is provided with sell-in value equal than 0",
			args: args{item: item.Item{Name: "Regular-evaluated item", SellIn: 0, Quality: 8}},
			want: want{quality: 6},
		},
		{
			name: "should decrease by 2 the quality value when an item is provided with sell-in value lower than 0",
			args: args{item: item.Item{Name: "Regular-evaluated item", SellIn: -1, Quality: 6}},
			want: want{quality: 4},
		},
		{
